...
from logprep.connector.confluent_kafka_shared import (ConfluentKafka, ConfluentKafkaFactory,
                                                      InvalidMessageError, UnknownOptionError)
...
class ConfluentKafkaInputFactory(ConfluentKafkaFactory):
    ...
    def create_from_configuration(configuration: dict) -> 'ConfluentKafkaInput':
        ...
        try:
            kafka_input = ConfluentKafkaInput(configuration['bootstrapservers'],
                                              configuration['consumer']['topic'],
                                              configuration['consumer']['group'],
                                              configuration['consumer'].get(
                                                  'enable_auto_offset_store', True))
        except KeyError as error:
            raise InvalidConfigurationError(f'Confluent Kafka Input: Missing configuration '
                                            f'parameter {str(error)}!') from error
    ...
    def _add_hmac_to(self, event_dict, hmac_target_field_name, raw_event):
        ...
        if hmac_target_field_name == "<RAW_MSG>":
            ...
            if received_orig_message is not None:
                ...
                self._output.store_failed(f"Couldn't find the hmac target field '{hmac_target_field_name}'",
        ...
        if not add_was_successful:
            self._output.store_failed(f"Couldn't add the hmac to the input event as the desired output "
